            ...
            ShaderSpecializationState specState)
        {
            ...
            for (int index = 0; index < textureCount; index++)
            {
                ...
                if (hostTexture != null && texture.Target == Target.TextureBuffer)
                {
                    ...
                    _channel.BufferManager.SetBufferTextureStorage(stage, hostTexture, bufferCache, texture.Range, bindingInfo, bindingInfo.Format, false);
        ...
        private bool CommitImageBindings(TexturePool pool, ShaderStage stage, int stageIndex, bool poolModified, ShaderSpecializationState specState)
        {
            ...
            for (int index = 0; index < imageCount; index++)
            {
                ...
                if (hostTexture != null && texture.Target == Target.TextureBuffer)
                {
                    ...
                    var bufferCache = _channel.MemoryManager.GetBackingMemory(descriptor.UnpackAddress()).BufferCache;
                    _channel.BufferManager.SetBufferTextureStorage(stage, hostTexture, bufferCache, texture.Range, bindingInfo, format, true);
